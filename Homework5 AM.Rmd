---
title: 'N741: Homework 5 Making Tables'
author: "Abby Mutic"
date: "March 14, 2017"
output:
  html_document:
...
t1 <- Wong %>%
  ...
  summarise(mediandays = median(days, na.rm = TRUE),
            ...
            mediandur = median(duration, na.rm = TRUE),
            q1dur = quantile(duration, 0.25, na.rm = TRUE),
            q3dur = quantile(duration, 0.75, na.rm = TRUE))
  
  knitr::kable(t1)
            
```
#now clean up column headings and add a title
```{r}
knitr::kable(t1,
             col.names = c("Sex",
                            "Median Days", "25th Qu. Days",
                            "75th Qu. Days", "Median Duration",
                           "25th Qu. Duration", "75th Qu. Duration"),
             caption = "Summary Statistics of Days and Duration by Sex")
```
...
t2 <- Wong %>%
  group_by(sex) %>%
  summarise(avgpiq = mean(piq, na.rm = TRUE),
            sdpiq = sd(piq, na.rm = TRUE),
            avgviq = mean(viq, na.rm = TRUE),
            sd = sd(viq, na.rm = TRUE))
  
  knitr::kable(t2,
               col.names = c("Sex",
                             "Mean Perf IQ", "SD Perf IQ",
                             "Mean Verbal IQ", "SD Verbal IQ"),
               caption="Summary Statistics of Performance and Verbal IQ by Sex")
```
...
#group by age
#find raw counts using n()
#compute relative % by dividing by ss
...
knitr::kable(t3,
             col.names = c("Age Groups",
                           "Frequency",
                           "Percent"),
             caption="Frequency Table for Age Groups")
```
...
```{r}
Regm1 <- lm(piq~age+sex, data=Wong)
sm1 <- summary(Regm1)
...
```{r}
Regm2 <- lm(piq~age+sex+days+duration, data=Wong)
sm1 <- summary(Regm2)
knitr::kable(sm1$coefficients,
...
# Analysis of Variance Table
 anova(Regm1,Regm2)
```
#Now put in a comparison table
```{r}
Regm1Regm2 <- anova(Regm1,Regm2)
row.names(Regm1Regm2) <- c("Model 1", "Model 2")
knitr::kable(Regm1Regm2,
             caption = "Compare Model 1 and Model 2")
```
...
```{r}
library(DT)
DT::datatable(t1, options = list(pageLength = 5))
```
My Github repository is located at [https://github.com/amutic/N741interactive](https://github.com/amutic/N741interactive)
### References
